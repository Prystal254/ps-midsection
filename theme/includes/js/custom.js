...
//--------------------
    ...
    if(scroller >= $(".sub-section.brands").offset().top){
        ...
        if(video_run_once == false){
            $("#myvideo").get(0).load()
            $("#myvideo").get(0).play()
            $(".ticker .dot .progress").removeClass("active");
            $(".ticker .dot .progress").eq(0).addClass("active");
            let currI = 1;
            setInterval(() => {
                $(".ticker .dot .progress").removeClass("active");
                $(".ticker .dot .progress").eq(currI).addClass("active");
                currI++;
                currI = currI % 5;
            }, 7000);
            video_run_once = true;
